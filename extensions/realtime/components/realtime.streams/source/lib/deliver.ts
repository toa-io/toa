...
/**
 * Pushes an event to every consumer of the key, and stashes it for the one that reconnects with a
 * token. The token that stashing answers follows the event to the consumers.
 */
export function deliver(context: Context, key: string, { event, data }: Message): void {
