...
export class Gateway extends Connector {
  ...
  public async process(context: http.Context): Promise<http.OutgoingMessage> {
    ...
    await context.timing
      .capture('preflight', this.directives.preflight(context))
      .catch(rethrow)
    ...
    await context.timing
      .capture('depart', this.directives.depart(context, response))
      .catch(rethrow)
  ...
  private async route(context: http.Context): Promise<http.OutgoingMessage> {
    ...
    const interruption = await context.timing
      .capture('precall', method.directives.precall(context, parameters))
      .catch(rethrow)
    ...
    await context.timing
      .capture('settle', method.directives.settle(context, response))
      .catch(rethrow)
