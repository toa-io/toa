...
export async function types(argv) {
  const written =
    argv.components === undefined ? await whole(argv) : await parts(argv.components)

  if (argv.quiet !== true) for (const file of written) console.log(file)
}
...
async function parts(patterns) {
  ...
  if (manifests.length === 0)
    throw new Error(`No component found in ${patterns.join(', ')}`)
