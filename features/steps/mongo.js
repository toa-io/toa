...
When('I make the {component} outbox rows due', async function (id) {
  await using(id, async (_, outbox) => outbox.updateMany({}, { $set: { pending: 0 } }))
})

Then('the {component} outbox is empty', async function (id) {
  await using(id, async (_, outbox) =>
...
Then('the {component} outbox holds {int} unpublished row(s)', async function (id, count) {
  await using(id, async (_, outbox) =>
...
Then(
  'the {component} outbox collection does not exist',
  /**
...
Then('the {component} outbox row carries an origin', async function (id) {
  await using(id, async (_, outbox) => {
    const [row] = await outbox.find({}).sort({ _id: -1 }).limit(1).toArray()

    assert.ok(row !== undefined, 'no outbox row')
    assert.ok(
      row.event.origin !== undefined && row.event.origin !== null,
...
Then(
  'the {component} record matches the last reply',
  /**
...
Then(
  'the {component} collection holds:',
  /**
   * @param {string} id
   * @param {import('@cucumber/cucumber').DataTable} table
   * @this {toa.features.Context}
...
Given(
  'the {component} migration {word} is recorded',
  /**
...
Then(
  'the {component} collection has indexes:',
  /**
  ...
  async function (id, table) {
    await using(id, async (collection) => {
      ...
      for (const { name, keys, unique, sparse } of table.hashes()) {
        ...
        if (unique !== undefined)
          assert.strictEqual(index.unique === true, unique === 'true')
        if (sparse !== undefined)
          assert.strictEqual(index.sparse === true, sparse === 'true')
      }
    })
  }
)
...
Then(
  'the {component} migrations are recorded:',
  /**
   * @param {string} id
   * @param {import('@cucumber/cucumber').DataTable} table
   * @this {toa.features.Context}
  ...
  async function (id, table) {
    await using(id, async (collection, _, db) => {
      const rows = await db
        ...
        .find({ _id: { $regex: `^${collection.collectionName}:` } })
        .toArray()

      for (const { migration, state } of table.hashes()) {
        const row = rows.find(
          ({ _id }) => _id === `${collection.collectionName}:${migration}`
        )
