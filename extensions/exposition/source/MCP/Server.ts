...
export class Server {
  ...
  public async process(
    ...
    if (modern) this.mirrored(context, message)
    ...
    if (message.id === undefined) return { status: ACCEPTED }
  ...
  private admit(context: http.Context): void {
    ...
    if (this.options.anonymous === true) return
    ...
    switch (message.method) {
      ...
      case 'tools/list':
        return this.complete(
          { tools: await list(this.tree, scope.context) },
          modern,
          TOOLS_CACHE
        )
      ...
      default:
        throw new http.NotFound(
          response(
            ...
            failure(
              METHOD_NOT_FOUND,
              `'${message.method}' is not a method this server answers`
            )
          )
        )
    }
  ...
  private async tool(scope: Scope, params: Params): Promise<object> {
    ...
    if (typeof args !== 'object' || args === null || Array.isArray(args))
      throw new http.BadRequest(
        response(null, failure(INVALID_PARAMS, '`arguments` must be an object'))
      )

    return await console.span(
      { name: named },
  ...
  private serverInfo(): unknown {
    return (this.discovery.modern as { _meta: Record<string, unknown> })._meta[
      SERVER_INFO
    ]
  }
  ...
  private async read(context: http.Context): Promise<Message> {
    ...
    if (
      ...
      body === null ||
      Array.isArray(body) ||
      (body as Message).jsonrpc !== JSONRPC ||
      typeof (body as Message).method !== 'string'
    )
      throw new http.BadRequest(
        response(null, failure(INVALID_REQUEST, 'A request carries one JSON-RPC message'))
      )
...
function mirrors(context: http.Context, mirror: Mirror): void {
  ...
  if (stated === undefined)
    throw new http.BadRequest(
      response(id, failure(HEADER_MISMATCH, `'${header}' is required`))
    )
...
function decode(value: string): string {
  if (!value.startsWith(SENTINEL) || !value.endsWith(TERMINATOR)) return value
