...
export class Factory implements extensions.Factory {
  ...
  /**
   * The probe answers for the process, so it is the process it belongs to — not to whichever
   * composition happens to be nearest, which in a service is the one nested inside it. It
   * binds before the process is built and answers `503` until `complete()`, and a halt leaves
   * it where it is: what a halt takes down is behind a gate, and this is not.
   */
