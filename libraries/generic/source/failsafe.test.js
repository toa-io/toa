...
import { failsafe } from '../source/index.js'
...
const fn = /** @type {import('node:test').Mock<any>} */ mock.fn(async () => generate())
const recover =
  /** @type {import('node:test').Mock<(...args: any[]) => Promise<any>>} */
  mock.fn(async () => true)
...
for (const [_, method] of [
  ...
  describe(`${_} recovery`, () => {
    ...
    it('should pass arguments', async () => {
      ...
      assert.ok(
        fn.mock.calls.some((call) => isDeepStrictEqual(call.arguments, [...args]))
      )
    })
...
describe('recovery function', () => {
  ...
  it('should throw on recovery failure', async () => {
    ...
    await assert.rejects(instance.doWithRecovery(), (error) => {
      assert.deepStrictEqual(error, exception)
      return true
    })
  })
  ...
  it('should pass exception to recover', async () => {
    ...
    assert.ok(
      recover.mock.calls.some(
        (call) =>
          call.arguments.length === 1 && isDeepStrictEqual(call.arguments[0], exception)
      )
    )
  })
...
describe('disable', () => {
  it('should throw exception', async () => {
    ...
    await assert.rejects(instance.doWithoutRecovery(), (error) => {
      assert.deepStrictEqual(error, exception)
      return true
    })
  })
