...
**It answers on every port it was listening on.** The readiness probe answers `200` — there is
nothing wrong with the process, and one that came up in its place would come up running — and the
gateway answers `503`, with `retry-after` stating when the deployment is back.
