import { Forbidden } from '../../HTTP/index.ts'
...
export class Input implements Directive {
  ...
  public constructor(declarations: Declaration[], create: Create) {
    this.statements = declarations.map(
      (declaration) => new Statement(declaration, create)
    )
  }
  ...
  public async authorize(
    ...
    context.pipelines.body.push(async (body) =>
      this.check(identity, context, parameters, body)
    )
  ...
  private async check(
    ...
    if (body === undefined || body === null || body.constructor !== Object) return body
...
class Statement {
  ...
  public async check(
    ...
    for (const directive of this.directives) {
      ...
      if (!authorized) throw new Forbidden('Input property is not authorized')
    }
  }
}
