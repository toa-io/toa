...
export async function effect(
  ...
  const query = {
    criteria: `authority==${quote(authority)};iss==${quote(iss)};sub==${quote(sub)}`
  }

  const asserted = newid()

  const credential =
    context.configuration.assert !== false
  ...
  if (credential === null) return ERR_NOT_FOUND
  ...
  if (credential.identity === asserted) await principal(credential, context)
